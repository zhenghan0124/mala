...
class GwfxController extends GlobalController {
  public function index(){
    $count = M('Gwfx') -> where($map) -> count();
    $Pages = new \Think\Page($count,6);// 实例化分页类 传入总记录数和每页显示的记录数(25)
    $show = $Pages->show();// 分页显示输出
    $result_arr = M('Gwfx') -> where($map) -> order('rank desc,time desc,id desc') -> limit($Pages -> firstRow.','.$Pages -> listRows) -> select();
   ...
   //  $this->assign('data1',$data1);
    $pic = D('Block') -> get_item_by_title('gwfx_banner');
