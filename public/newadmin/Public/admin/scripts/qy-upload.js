      ...
      $(this).parent().ajaxSubmit({
        ...
        success: function(data) { //成功
          if(data.status == 1){
            ...
            //判断是否上传多张图片
            if(multiple == 0){
              thisObj.parent().before('<li><img src="/newadmin/Uploads/'+data.path+data.pic+'" /><input type="hidden" name="'+name+'" value="'+data.path+data.pic+'" /><span class="qy-upload-del"><i class="fa fa-remove"></i></span></li>');
              thisObj.parent().hide();
            }else{
              thisObj.parent().before('<li><img src="/newadmin/Uploads/'+data.path+data.pic+'" /><input type="hidden" name="'+name+'[]" value="'+data.path+data.pic+'" /><span class="qy-upload-del"><i class="fa fa-remove"></i></span></li>');
            }
